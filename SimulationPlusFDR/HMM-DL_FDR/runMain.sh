...
perl SeqThreshold.pl $input.dlmax.txt > $input.dlmaxseq.txt
echo "testmapEC.pl"
perl testmapEC.pl $input.dlmaxseq.txt > $input.dlmaxseqcount.txt
echo "countdomain.pl"
perl countdomain.pl  $input.dlmaxseqcount.txt

rm $input.parse.txt $input.sort.txt $input.GA.txt $input.dln.txt $input.dlmax.txt $input.dlmaxseq.txt $input.dlmaxseqcount.txt
